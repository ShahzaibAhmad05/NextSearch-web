    ...
    <Card className={`border backdrop-blur-sm hover:shadow-lg transition-all duration-300 ${
      ...
      <div className="flex items-start gap-4">
        ...
        <div className={`shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-lg ${
          isReplyable 
            ? 'bg-linear-to-br from-indigo-500/30 to-purple-500/30 text-indigo-200 border-2 border-indigo-400/50' 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm ${
                isReplyable 
                  ? 'bg-linear-to-r from-indigo-500/20 to-purple-500/20 text-indigo-300 border border-indigo-400/30' 
          ...
          <div className={`p-4 rounded-lg mb-3 ${
            isReplyable ? 'bg-slate-800/40 border border-indigo-500/10' : 'bg-slate-800/30 border border-slate-700/30'
          }`}>
          ...
          {feedback.email && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
              isReplyable ? 'bg-indigo-500/10 border border-indigo-500/20' : 'bg-slate-800/50 border border-slate-700/30'
            }`}>
              <svg className={`w-4 h-4 ${isReplyable ? 'text-indigo-400' : 'text-slate-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <p className={`text-xs font-medium truncate ${
                isReplyable ? 'text-indigo-300' : 'text-slate-400'
              }`}>
                {feedback.email}
              </p>
            </div>
